 ...
 * Video Explanation : https://www.youtube.com/watch?v=v0e8p9JCgRc
 *
 ...
 *
 */
public class _10_SumOfSubArrayMinimums {
     ...
     * This will give TLE due to n^2 time complexity.
     *
     * Time Complexity: O(n²)
     *
     * Analysis:
     ...
     * - For each iteration i, the inner while loop processes all elements currently in the stack
     * - In the worst case, the stack contains all previous elements (0 to i-1)
     * - This means for iteration i, we do i operations in the inner loop
     * - Total operations: 1 + 2 + 3 + ... + n = n(n+1)/2 = O(n²)
     *
     * Worst case scenario: A strictly increasing array like [1, 2, 3, 4, 5]
     * At each step, all previous elements remain in the stack
     * Each new element requires processing all previous elements
     *
     * Space Complexity: O(n)
     * Analysis:
     * - Main stack: In worst case, stores all n indices → O(n)
     * - Temp stack: In worst case, stores all elements from main stack → O(n)
     * - Other variables: answer, element, min, index, i → O(1)
    ...
    * */
    ...
    /**
     * Solution 2 : Brute Force Approach using 2 loops
     * This will also give TLE due to n^2 time complexity but the code is much simpler
     * compared to our stack based solution in solution 1.
     *
     * Time Complexity: O(n²)
     * - Outer loop: n iterations
     * - Inner loop: worst case (n-1) iterations
     * - Total: n + (n-1) + (n-2) + ... + 1 = n(n+1)/2 = O(n²)
     *
     * Space Complexity: O(1)
     * - Only uses a few variables (answer, min, i, j)
     ...
     *
     * previousSmallerOrEqualEleIndexes(): 1n - each element pushed/popped once
     * nextSmallerEleIndexes(): 1n - each element pushed/popped once
     * Main loop: 1n - single pass for calculations
     * Arrays.fill() calls: 2n - filling two arrays of size n
     * Total: 4n → O(n)
     *
     * Space Complexity: O(n) = 3n
     *
     * previousSmallerElementsArr: 1n - result array
     * nextSmallerElementsArr: 1n - result array
     * Stack storage: 1n - worst case when monotonic (strictly increasing/decreasing input)
     * Total: 3n → O(n)
    * */
    public int sumSubarrayMins2(int[] arr) {
        long answer = 0;
        int[] previousSmallerElementsArr = previousSmallerOrEqualEleIndexes(arr);
        int[] nextSmallerElementsArr = nextSmallerEleIndexes(arr);
        for (int i = 0; i < arr.length; i++) {
            int leftElements = i - previousSmallerElementsArr[i];
            int rightElements = nextSmallerElementsArr[i] - i;
            long sum = ((long) leftElements * rightElements * arr[i]);
            answer+=sum;
        }
        return (int) (answer%MOD);
    }
